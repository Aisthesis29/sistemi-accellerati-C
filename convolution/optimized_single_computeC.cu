    ...
    if(y_local >= borders) {
        ...
        for (int dy = y-radius; dy < y; ++dy) {
         ...
         __sad(val_gU, center_g,
               abs(val_bU - center_b)));
                w = w_s * expf(-(dr*dr) * inv_2_sigma_r2);
                wsum += w;
         ...
         __sad(val_gD, center_g,
               abs(val_bD - center_b)));
                w = w_s * expf(-(drD*drD) * inv_2_sigma_r2);;
                wsum += w;
        ...
        for(int dx=x0; dx<=xn; dx++) {
         ...
         __sad(val_g, center_g,
               abs(val_b - center_b)));
            float w = space_weight[(dx-x+radius)*dim_kernel+radius] * expf(-(dr*dr) * inv_2_sigma_r2);;
        ...
        for (int dy = 0; dy <= y_local+radius; ++dy) {
         ...
         __sad(val_g, center_g,
               abs(val_b - center_b)));
                float w = space_weight[(dx-x+radius)*dim_kernel+(dy-y_local+radius)] * expf(-(dr*dr) * inv_2_sigma_r2);;
        ...
        for (int dy = y-radius; dy <= height-1; ++dy) {
         ...
         __sad(val_g, center_g,
               abs(val_b - center_b)));
                float w = space_weight[(dx-x+radius)*dim_kernel+(dy-y+radius)] * expf(-(dr*dr) * inv_2_sigma_r2);;
