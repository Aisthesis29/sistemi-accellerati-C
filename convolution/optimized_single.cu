    ...
    if(y_local >= borders) {
        ...
        for (int dy = y-radius; dy < y; ++dy) {
            for (int dx = x0; dx <= xn; ++dx) {
                ...
                //wsum += w;
                wsum=fmaf(w_s,color_weight[dr],wsum);
                ...
                //wsum += w;
                wsum=fmaf(w_s,color_weight[drD],wsum);
...
//            wsum += w;
            wsum=fmaf(space_weight[(dx-x+radius)*dim_kernel+radius],color_weight[dr],wsum);
        ...
        for (int dy = 0; dy <= y_local+radius; ++dy) {
            for (int dx = x0; dx <= xn; ++dx) {
                ...
                //wsum += w;
                wsum=fmaf(space_weight[(dx-x+radius)*dim_kernel+(dy-y_local+radius)],color_weight[dr],wsum);
        ...
        for (int dy = y-radius; dy <= height-1; ++dy) {
            for (int dx = x0; dx <= xn; ++dx) {
                ...
                //wsum += w;
                wsum=fmaf(space_weight[(dx-x+radius)*dim_kernel+(dy-y+radius)],color_weight[dr],wsum);
