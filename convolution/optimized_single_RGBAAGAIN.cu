...
int main(int argc, char **argv) {
    ...
    uchar4* h_input = stbi_load(inputFile, &width, &height, &channels, 4);
        unsigned char *h_input_CPU = stbi_load(inputFile, &width, &height, &channels, 3);
    ...
    bilateral_u8_gray<<<grid, block>>>(d_input, d_output, width,height,2*radius,inner_rows, radius, d_color_weight, dim_kernel);
    /*int rows =radius;
    ...
    bilateral_u8_gray_cpu(h_input_CPU, h_output_cpu, width, height, radius, sigma_s, sigma_r);
